...
5. **极其关键：** 为了在前端准确显示本题的原图，请你估算每道题在整页图片中垂直方向的高度范围。在返回的每个题目 JSON 中，必须包含 \`image_y_start\` 和 \`image_y_end\` 两个字段。取值为 0 到 100 的整数，代表百分比（0为最顶部，100为最底部）。例如：如果题目位于图片最顶端并占大约五分之一，返回 \`"image_y_start": 0, "image_y_end": 20\`。请尽量让这个高度范围包含题干、图形和选项。如果这道题跨越了多半个屏幕，你可以返回 \`"image_y_start": 40, "image_y_end": 80\`。
